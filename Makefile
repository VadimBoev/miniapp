# Thanks to CnLohr for a good Makefile example
# adapted on C++ by Vadim Boev
...
LABEL?=$(APPNAME)
APKFILE ?= $(APPNAME).apk
PACKAGENAME?=boev.lifeit.$(APPNAME)
RAWDRAWANDROID?=.
RAWDRAWANDROIDSRCS=$(RAWDRAWANDROID)/vendor/android_native_app_glue/android_native_app_glue.c
SRC?=./*.cpp
...
CFLAGS?=-ffunction-sections -Os -fdata-sections -Wall -fvisibility=hidden -pthread
...
makecapk.apk : $(TARGETS) $(EXTRA_ASSETS_TRIGGER) AndroidManifest.xml
	...
	rm -rf temp.apk
	rm -rf makecapk.apk
	rm makecapk/lib/armeabi-v7a/lib$(APPNAME).so
	@ls -l $(APKFILE)
